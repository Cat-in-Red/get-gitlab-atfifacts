...
_get_artifacts() {
    ...
    _resp=$(curl -sS -k -q -f -LL --header "PRIVATE-TOKEN: $PRIVATE_TOKEN" "${CI_API_V4_URL}/projects/$CI_PROJECT_ID/jobs?scope=success")
    if [ -z "$_resp"  ]; then
        return 1
    fi

    _job_id=$(echo "$_resp" | jq --arg _job_name "$_job_name" '[.[] | select(.name == $_job_name)] |.[0].id')
    if [ -z "$_job_id" ] || [ "$_job_id" = "null" ]; then
        ERROR="Not found successfully jobs with name $_job_name"
        return 1
    fi

    _status=$(curl -sS -k -q -LL -w "%{http_code}"  -o "$ARTIFACT_FILE" --header "PRIVATE-TOKEN: $PRIVATE_TOKEN" "${CI_API_V4_URL}/projects/$CI_PROJECT_ID/jobs/$_job_id/artifacts")
    if [ $_status != 200 ]; then
